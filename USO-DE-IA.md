...
- [x] **Erro factual encontrado e corrigido na revisão:** a IA havia indicado a
      ...
      `returntocorp` é a organização antiga do **Semgrep**, não do OpenGrep —
      corrigido para a imagem da org `opengrep`. Registrado aqui como evidência
      de que o material gerado passa por conferência, e não é aceito como veio
...
- [ ] Executar todos os comandos do `LAB.md` contra o DVWA local e **substituir
      os blocos de output de exemplo pelos outputs reais**
- [ ] Confirmar nome e tag reais das imagens Docker com `docker pull`
      (`opengrep/opengrep`, `hysnsec/nikto`, `vulnerables/web-dvwa`)
